...
class MessyTableTripletDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        samples_ap_subcls = []
        samples_n_subcls = []
        ## Add camera ID and position information in data generator ##
        samples_main_cam = []
        samples_sec_cam = []
        samples_a_bbox = []
        samples_p_bbox = []
        ...
        for trip in triplets_list:
            ...
            ## Add camera ID and position information in data generator ##
            samples_main_cam.append(int(trip['main_cam']))
            samples_sec_cam.append(int(trip['sec_cam']))
            samples_a_bbox.append(a_bbox)
            samples_p_bbox.append(p_bbox)
            samples_n_bbox.append(n_bbox)
        ...
        ## Add camera ID and position information in data generator ##
        samples['main_cam'] = torch.from_numpy(np.array(samples_main_cam))
        samples['sec_cam'] = torch.from_numpy(np.array(samples_sec_cam))
        samples['a_bbox'] = torch.from_numpy(np.array(samples_a_bbox))
        samples['p_bbox'] = torch.from_numpy(np.array(samples_p_bbox))
        samples['n_bbox'] = torch.from_numpy(np.array(samples_n_bbox))
        #  samples['p_bbox'] = torch.stack(samples_p_bbox)
        #  samples['n_bbox'] = torch.stack(samples_n_bbox)
...
class MessyTableDatasetFeatures(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #  ## Add IoU information ##
        #  temp_IoU_list = []
        #  if 'IoU_filter' in self.config:
        #      IoU_filter = self.config['IoU_filter']
        #  else:
        #      IoU_filter = False
        ...
        for cam, cam_value in content['cameras'].items():
            ...
            for inst_id, inst_value in cam_value['instances'].items():
                ## Add cls information ##
                temp_attr_list.append([int(cam), int(inst_id), inst_value['subcls'], inst_value['cls']])  # added subcls
                ...
                #  ## Add IoU information ##
                #  if IoU_filter:
                #      temp_IoU_list.append(inst_value['iou'])
        ...
        ## Outdated ##
        #  ## Add IoU information ##
        #  info['iou'] = np.array(temp_IoU_list)
...
class MessyTableDatasetCompare(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ## Add cls information ##
        main_cam_cls = self.feat_vec_dict[scene_name][main_cam]['cls']
        sec_cam_cls = self.feat_vec_dict[scene_name][sec_cam]['cls']
        ...
        ## Add cls information ##
        for main_inst_id, main_subcls, main_cls in zip(main_cam_inst_id, main_cam_subcls, main_cam_cls):
            for sec_inst_id, sec_subcls, sec_cls in zip(sec_cam_inst_id, sec_cam_subcls, sec_cam_cls):
                gt = 1 if main_inst_id == sec_inst_id else 0
                gt_subcls = 1 if main_subcls == sec_subcls else 0
                ## Add cls information ##
                gt_cls = 1 if main_cls == sec_cls else 0
                ...
                #  ## Add IoU information ##
                #  if IoU_filter:
                #      main_cam_IoU = self.feat_vec_dict[scene_name][main_cam]['iou'][main_inst_index]
                #      IoU_filter_list.append(main_cam_IoU)
            main_inst_index += 1
        ...
        info = {
            ...
            ## Add cls information ##
            'gt_cls': torch.from_numpy(np.array(gt_cls_list).astype(np.float32)).reshape((-1, 1)),
            ...
            ## Outdated ##
            #  ## Add IoU information ##
            #  'main_IoU': torch.from_numpy(np.array(IoU_filter_list).astype(np.float32)).reshape((-1, 1))
        }
