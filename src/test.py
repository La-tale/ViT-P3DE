...
def retrieve_features(model_app, model_neighbor, feature_loader, config):
    ...
    ## Outdated ##
    #  ## ## Add IoU information ##
    #  if 'IoU_filter' in config:
    #      IoU_filter = config['IoU_filter']
    #  else:
    #      IoU_filter = False
    ...
    with torch.no_grad():
        for sample_dict in feature_loader:
            ...
            if 'single_crop' in config['zoomout_crop_num']:
                ...
                ## Add camera ID & position of object for P3DEs ##
                cam_index = sample_dict['instance_attributes'][0,:,0].to(config['device']) 
                pos = sample_dict['instance_pos'][0].to(config['device'])

                if config['mode'] == 'Triplet':
                    app_features = model_app.forward_once(single_crop)
                elif config['mode'] == 'Triplet_cam':
                    app_features = model_app.forward_once(single_crop,cam_index)
                elif config['mode'] == 'Triplet_cam_pos':
                    app_features = model_app.forward_once(single_crop,cam_index,pos)
                else:
            ...
            ## Outdated ##
            #  ## Add IoU information ##
            #  feature_vector_dict[scene_name]['iou'] = sample_dict['iou']
    ...
    for scene_name, scene_content in feature_vector_dict.items():
        ...
        for j in range(num_instances):
            ## Add cls information ##
            cam, inst_id, subcls, cls = feature_vector_dict[scene_name]['instance_attributes'][j, :]
            cam = str(cam.numpy())
            ...
            ## Add cls information ##
            feature_vector_cam_dict[scene_name][cam]['cls'].append(cls)
            ...
            ## Add IoU information ##
            #  if IoU_filter:
            #      feature_vector_cam_dict[scene_name][cam]['iou'].append((feature_vector_dict[scene_name]['iou'][0,j].cpu().detach().numpy()).item(0))
...
def eval_model(config, turn_on_save=True):
    ...
    ## Load file according to difficulty ## 
    file_name = dev_label_pathname.split('/')[-1]
    if '_' not in file_name:
        difficulty=''
    else:
        difficulty= '_'+file_name.split('_')[-1].split('.')[0]
    ...
    if not config['load_features']:
            ...
            for sample_dict in compare_loader:
                ...
                results_to_save[img_pair] = {
                    ...
                    'gt_subcls': [str(i) for i in list(sample_dict['gt_subcls'][0].cpu().detach().numpy().squeeze())],
                     ## Add cls information ##
                    'gt_cls': [str(i) for i in list(sample_dict['gt_cls'][0].cpu().detach().numpy().squeeze())],
                    ...
                    #  ## Add IoU information ##
                    #  'iou': [str(i) for i in
                    #                  list(sample_dict['main_IoU'][0].cpu().detach().numpy().squeeze())]
                }
                ...
                ## Add cls information ##
                gt_cls_list += gt_cls.cpu().detach().numpy().squeeze().tolist()
                ...
                ## Outdated ##
                #  ## Add IoU information ##
                #  main_iou_list += main_iou.cpu().detach().numpy().squeeze().tolist()
        ...
        ## Load file according to difficulty ## 
        results_img_pairs_pathanme = os.path.join(config['config_dir'], 'results_img_pairs'+difficulty+'.json')
        app_dist_pathname = os.path.join(config['config_dir'], 'app_dist'+difficulty +'.npy')
        gt_inst_np_pathname = os.path.join(config['config_dir'], 'gt_inst_np'+difficulty+'.npy')
    ...
    if config['save_features']:
        ...
        app_dist_pathname = os.path.join(config['config_dir'], 'app_dist'+difficulty+'.npy')
        epi_dist_pathname = os.path.join(config['config_dir'], 'epi_dist'+difficulty+'.npy')
        angle_diff_pathname = os.path.join(config['config_dir'], 'angle_diff'+difficulty+'.npy')
        gt_inst_np_pathname = os.path.join(config['config_dir'], 'gt_inst_np'+difficulty+'.npy')
...
## Eval SPS / SPD, SBS / SBD ##
def eval_by_subncls(config):
    device = config['device']
    dev_label_pathname = config['eval_pathname']
    ## Load file according to  difficulty ## 
    file_name = dev_label_pathname.split('/')[-1]
    ...
    ### compute metrics: AP, IPAA
    gt_inst_multi_subcls_list = []
    gt_inst_single_subcls_list = []

    gt_inst_multi_cls_list = []
    gt_inst_single_cls_list = []
    numel = 0
    for scene_pair, val in results_to_save.items():
        main_num = len(val['main_bbox_id'])
        sec_num = len(val['sec_bbox_id'])

        gt_inst = np.reshape(np.array(list(map(float,val['gt_inst']))),(main_num,sec_num))
        gt_subcls = np.reshape(np.array(list(map(float,val['gt_subcls']))),(main_num,sec_num))
        gt_cls = np.reshape(np.array(list(map(float,val['gt_cls']))),(main_num,sec_num))

        subcls_multi_cond = np.unique(np.where(gt_inst != gt_subcls)[0])
        subcls_single_cond = np.setdiff1d(np.arange(main_num),subcls_multi_cond)
        subcls_multi_cond_reshape = (np.repeat(subcls_multi_cond,sec_num) * sec_num + numel) + np.tile(np.arange(sec_num),subcls_multi_cond.shape[0])
        subcls_single_cond_reshape = (np.repeat(subcls_single_cond,sec_num) * sec_num + numel) + np.tile(np.arange(sec_num),subcls_single_cond.shape[0])

        cls_multi_cond = np.unique(np.where(gt_inst != gt_cls)[0])
        cls_single_cond = np.setdiff1d(np.arange(main_num),cls_multi_cond)

        cls_multi_cond_reshape = (np.repeat(cls_multi_cond,sec_num) * sec_num + numel) + np.tile(np.arange(sec_num),cls_multi_cond.shape[0])
        cls_single_cond_reshape = (np.repeat(cls_single_cond,sec_num) * sec_num + numel) + np.tile(np.arange(sec_num),cls_single_cond.shape[0])

        assert len(cls_multi_cond_reshape) + len(cls_single_cond_reshape) == main_num*sec_num 
        assert len(subcls_multi_cond_reshape) + len(subcls_single_cond_reshape) == main_num*sec_num

        gt_inst_multi_subcls_list.append(subcls_multi_cond_reshape)
        gt_inst_single_subcls_list.append(subcls_single_cond_reshape)
        gt_inst_multi_cls_list.append(cls_multi_cond_reshape)
        gt_inst_single_cls_list.append(cls_single_cond_reshape)

        numel += main_num*sec_num

    gt_inst_multi_subcls_np = np.concatenate(gt_inst_multi_subcls_list)
    gt_inst_single_subcls_np = np.concatenate(gt_inst_single_subcls_list)

    gt_inst_multi_cls_np = np.concatenate(gt_inst_multi_cls_list)
    gt_inst_single_cls_np = np.concatenate(gt_inst_single_cls_list)

    assert numel == len(gt_inst_np) 
    assert len(gt_inst_np) == len(gt_inst_multi_cls_np) + len(gt_inst_single_cls_np)
    assert len(gt_inst_np) == len(gt_inst_multi_subcls_np) + len(gt_inst_single_subcls_np)

    print('Multi subcls')
    gt_inst_cond_np = gt_inst_np[gt_inst_multi_subcls_np]
    dist_cond_np = dist_np[gt_inst_multi_subcls_np]
    score = 1 - scale_data(dist_cond_np)
    inst_AP = cal_mAP(score, gt_inst_cond_np)
    fpr = FPR_95(dist_cond_np, gt_inst_cond_np)
    print('AP = {:0.3f}'.format(inst_AP))
    print('FPR = {:0.3f}'.format(fpr))
    print('\n')
    
    print('Single subcls')
    gt_inst_cond_np = gt_inst_np[gt_inst_single_subcls_np]
    dist_cond_np = dist_np[gt_inst_single_subcls_np]
    score = 1 - scale_data(dist_cond_np)
    inst_AP = cal_mAP(score, gt_inst_cond_np)
    fpr = FPR_95(dist_cond_np, gt_inst_cond_np)
    print('AP = {:0.3f}'.format(inst_AP))
    ...
    print('Multi cls')
    gt_inst_cond_np = gt_inst_np[gt_inst_multi_cls_np]
    dist_cond_np = dist_np[gt_inst_multi_cls_np]
    score = 1 - scale_data(dist_cond_np)
    inst_AP = cal_mAP(score, gt_inst_cond_np)
    fpr = FPR_95(dist_cond_np, gt_inst_cond_np)
    print('AP = {:0.3f}'.format(inst_AP))
    ...
    print('Single cls')
    gt_inst_cond_np = gt_inst_np[gt_inst_single_cls_np]
    dist_cond_np = dist_np[gt_inst_single_cls_np]
    score = 1 - scale_data(dist_cond_np)
    inst_AP = cal_mAP(score, gt_inst_cond_np)
    fpr = FPR_95(dist_cond_np, gt_inst_cond_np)
    print('AP = {:0.3f}'.format(inst_AP))
...
def eval_by_angle(config, mode='model_only'):
    ...
    FPR_list = []
    for itm in angle_partitions:
        ## Don't evaluate if no element exist in group ##
        if gt_inst_angle_dict[itm].shape[0] >0: 
    ...
    ## Load file according to difficulty dependency ## 
    file_pathname = os.path.join(config['config_dir'], 'angle_partionted_results'+difficulty+'.json')
    if mode == 'model_esc':
        file_pathname = os.path.join(config['config_dir'], 'angle_partionted_results_esc'+difficulty+'.json')
    results_dict = {
...
def main(args):
    ...
    ## Eval SPS / SPD, SBS / SBD
    config['eval_by_subncls'] = args.eval_by_subncls
    ...
    ## Eval SPS / SPD, SBS / SBD
    if config['eval_by_subncls']:
        print('----evaluating by classes with filtering...')
        eval_by_subncls(config)
    toc = time.time()
...
if __name__ == '__main__':
    ...
    ## Eval SPS / SPD, SBS / SBD
    parser.add_argument('--eval_by_subncls', action='store_true',
                        help='option to eval model by cls filtering')
