...
def parse_config(args):
    ...
    if hasattr(args,'train_label_pathname'):
        if args.train_label_pathname:
            config['train_label_pathname'] = os.path.join(config['data_dir'],'labels',args.train_label_pathname)
        else:
    ...
    # make cam_pairs
    config['cam_indexes'] = config['cam_indexes'] if 'cam_indexes' in config else [1,2,3,4,5,6,7,8,9]
    train_cam_pairs = []
    for i in range(len(config['cam_indexes'])):
        for j in range(i+1, len(config['cam_indexes'])):
            train_cam_pairs.append((str(config['cam_indexes'][i]),str(config['cam_indexes'][j])))
    ...
    reversed_cam_pairs = []
    for cam_pair in train_cam_pairs:
        reversed_cam_pairs.append((cam_pair[1],cam_pair[0]))

    config['test_cam_pairs'] = test_cam_pairs

    fixed_train_cam_pairs = []
    if 'train_cam_pairs' in config:
        for i in range(0,len(config['train_cam_pairs']),2):
            fixed_train_cam_pairs.append((str(config['train_cam_pairs'][i]),str(config['train_cam_pairs'][i+1])))
        config['train_cam_pairs'] = fixed_train_cam_pairs
    else:
        config['train_cam_pairs'] = train_cam_pairs + reversed_cam_pairs

    #  config['train_cam_pairs'] = config['train_cam_pairs'] if 'train_cam_pairs' in config else train_cam_pairs + reversed_cam_pairs
